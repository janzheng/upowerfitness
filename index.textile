---
layout: framework
title: upower fitness - harnessing the power of you.
comments: false
---
<div id="index-container">


<article id="intro" class="goto">
<div id="mobile-banner"></div>
<section id="intro-slider">
   <div class="flex-container">
      <div class="flexslider">
         <ul class="slides">
            <li id="slide1">

               <section class="container-section">
                  <div class="flex-caption">
                     <h1 class="lighter">Power from Within </h1>
                     <p class="caption">Fitness is neither a fad, diet, nor a New Year's resolution. 

                        It is a lifestyle, a permanent resolution of sweat, nutrition, and self-control.
                     </p>
                  </div>
               </section>
            </li>
            <li id="slide2">

               <section class="container-section">
                  <div class="flex-caption">
                     <h1 class="lighter">Piedmont Park </h1>
                     <p class="caption">Uniquely located on the edge of Piedmont Park, we create workouts that combine both indoor and outdoor workouts to push your limits.
                  </div>
               </section>

            </li>
            <li id="slide3">

               <section class="container-section">
                  <div class="flex-caption">
                     <h1 class="lighter">Full-Body Workout </h1>
                     <p class="caption">We're equipped from anything from weights to spin cycles to ensure a full-body workout.
                  </div>
               </section>

            </li>
         </ul>
      </div>
   </div>
</section>


<section class="container-section" id="intro-section">

h1(green). More Than Just a Gym

p(section-intro orange). We're not just your neighborhood gym.
<br/>
We are the first step to helping you find and maintain your inner power and motivation, through strength and cardio conditioning, meal planning, and consultation. 

<div class="section-trio mobile-open" id="tri1"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. Conditioning


<div class="mobile-hide">
p. We condition you so your heart pumps harder and your lungs breathe deeper, so you can do the things you move longer and faster without getting tired
</div>
</div>
<div class="section-trio mobile-open" id="tri2"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. Meal Tracking

<div class="mobile-hide">
p. The say fitness is 70% nutrition and 30% gym. We watch what you eat, and ease you into healthy habits.
</div>
</div>
<div class="section-trio mobile-open" id="tri3"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. Strength Training

<div class="mobile-hide">
p(mobile-hide). We build your core strength so you can do all the things you've always wanted to do.
</div>
</div>

</section>
</article>




<article class="goto" id="training">
<div class="container" id="training-content" >
<section class="container-section" >

h1(green). Take Control with Personal Training

p(section-intro soft). Our personal training sessions include cardio, strength, and meal tracking. Each session combines both cardio and strength for a high interval workout.
<br />
Costs are listed per person, per session.
</section>

<section class="container-section" id="training-prices">

table(training-table).
|_\3(td-header).  One on One Training                       |
|(td-cat).           |_(td-sub).60 min.  |_(td-sub).30 min. |
|(td-cat).  1 Pack   |$65          |$45                     |
|(td-cat). 10 Pack   |$55          |                        |
|(td-cat). 20 Pack   |$45          |$35                     |
|(td-cat). 30 Pack   |$35          |$25                     |
|(td-cat). 40 Pack   |$25          |                        |

table(training-table).
|_\3(td-header).  u-Buddy Training                    |
|(td-cat).           |_(td-sub).2 Person  |_(td-sub).3 Person  |
|(td-cat).  1 Pack   |$32.50         |$22.50          |
|(td-cat). 10 Pack   |$27.50         |$19.00          |
|(td-cat). 20 Pack   |$22.50         |$15.00          |
|(td-cat). 30 Pack   |$17.50         |$12.00          |
|(td-cat). 40 Pack   |$12.50         |$9.50           |
|\3(td-note). note: all u-buddy classes last one hour          |

</section>

{% include assessments.html %}

</div>
</article>


<article class="goto" id="classes">
<div class="container" id="classes-content" >
<section class="container-section" >

h1(green). Come Strong, Leave Empowered

p(section-intro cloud). We also teach group cardio and conditioning classes. 
<br/>
Our classes are kept small so we can attend to each student's workout. We focus on strength and high interval training, while emphasizing form and control.



<div class="section-trio classes-trio mobile-open"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. U-Blast

<div class="mobile-hide">
p(class-time). 50 minutes

p(class-desc). Combining strength and cardio with interval training, you will spend 25 minutes inside the studio with strength training and 25 minutes of cardio in the park.

<section class="container-section classes-prices" >
table(classes-table).
|(td-cat). 1 Pack   |$20  |
|(td-cat). 5 Pack   |$15  |
|(td-cat). 20 Pack  |$10  |
</section>

<div class="day-schedule">
p(day). Tuesday

p(time). 7am - 8am
</div>

<div class="day-schedule">
p(day). Thursday

p(time). 5:30pm - 6:30pm
</div>

<div class="day-schedule">
p(day). Saturday

p(time). 10am - 11am
</div>

</div>
</div>
<div class="section-trio classes-trio mobile-open"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. U-Spin

<div class="mobile-hide">
p(class-time). 45 minutes

p(class-desc). Fun and energetic class of 45 minutes of spin cardio including sprints, heavy hill climbs, seated climbs and running jogs, to a blend of energetic, fun of music.

<section class="container-section classes-prices" >
table(classes-table).
|(td-cat). 1 Pack   |$20  |
|(td-cat). 10 Pack  |$10  |
|(td-cat). 15 Pack  |$10  |
</section>

<div class="day-schedule">
p(day). Monday

p(time). 6pm - 6:45pm
</div>

<div class="day-schedule">
p(day). Friday

p(time). 9am - 10am
</div>

</div>
</div>
<div class="section-trio classes-trio mobile-open"> 

<notextile><!-- !http://placehold.it/80x80! --></notextile>

h2. U-Spin + Body

<div class="mobile-hide">

p(class-time). 1 hour

p(class-desc). The best of both worlds, this class combines both spin and strength training for a tough one-hour workout, challenging both  your strength, endurance and cardio.

<section class="container-section classes-prices" >
table(classes-table).
|(td-cat). 1 Pack   |$20  |
|(td-cat). 5 Pack  |$15  |
|(td-cat). 10 Pack  |$10  |
</section>

<div class="day-schedule">
p(day). Wednesday

p(time). 7am - 8am
</div>


</div>

</section>

{% include assessments.html %}

</div>
</article>








<article id="about" class="goto">
<div class="container" id="about-content" >
<section class="container-section" id="about-1">

h1(green). Humble Beginnings

p(soft). For most people, losing weight is not easy. It takes dedication and permanent changes most people are unwilling to make. Diets, boot camps, and other solutions intended to "lose weight quickly" are neither sustainable nor realistic. 
<br />
In reality, eating a healthy diet and staying active requires a change in lifestyle that is neither drastic, nor hard to maintain. But it does take hard work and commitment.
<br />
upower fitness was founded on the idea that - if the desire is there, we can make it happen. We take a unique look at each client, and crafts a plan that helps them attain the goal that's right for the individual.
</section>
<section class="container-section" id="about-2">

h1(light). Power to the Community

p(section-intro ). upower fitness is not just about improving your own life, but also about improving lives of others and the life of the community itself. 
<br />
We partner with a number of local non-profit organizations around Atlanta, and once a month, the proceeds of a u-blast class will go towards a select charity.

</section>
<section class="container-section" id="biobabette">

p(quote). &ldquo;What better way to live your life than by turning your life's passion into your life's profession?&rdquo;

<notextile><img src="assets/images/trainer1.png" id="babetteimg" alt=""></notextile>

p(trainer). Babette Lombardo
%(qual)Founder and Head Trainer
ISSA certified personal trainer, CPR/AED certified%

p(trainer-para). Babette Lombardo, the founder and head trainer of upower Fitness has been able to accomplish this by turning her lifelong love of fitness, athletics and helping others into the movement that is upower fitness.
<br />
Babette grew up in a family that always encouraged her to follow her passion and work as hard as she could at everything she did. Her passion for tennis won her an full athletic scholarship at Jacksonville State University, where she earned her Bachelor of Arts degree. Moving to Atlanta in 2008 after graduation, she worked several years in sports marketing. While her love of athletics never waned and her desire to help others in some meaningful way continued to grow stronger, Babette began to realize her true calling. As her charitable work increased with Kate's Club and her fitness and cycle class instruction took off, Babette searched for ways to combine her desire to help with her passion for healthy living. 
<br />
After some time and a lot of discussion with family and friends, the concept of upower Fitness was born. Babette is eager to share her wealth of information about living a healthy, balanced life while simultaneously giving back to the community.

</div>
</section>


<section class="container-section" id="bionoel">

p(quote). &ldquo;Noel's Pull-Quote&rdquo;

<notextile><img src="assets/images/trainer21.jpg" id="noelimg" alt=""></notextile>

p(trainer2). Noel Milanese
%(qual)NASM Certified%

p(trainer-para). Upon joining upower fitness, Noel traded stilettos for sneakers to kick-start her dream of motivating others to live fit and healthy lifestyles.
<br />
Noel was first inspired by a fitness class at a local gym and was instantly drawn to the world of fitness. As a child who struggled with eating disorders and health issues, Noel realized that the importance of a healthy life starts at a young age. Her goals are to teach people how to eat a clean, healthy diet and how to exercise so that people develop healthy habits. Noel is certified through NASM and has published an e-book cookbook. 
<br />
upower is the perfect place for Noel to teach others that they have the power to make those positive changes in their lives. Noel is motivated to helping clients such as busy moms, dads, kids and everyone in between realize that small steps lead changes in lifestyle habits.

</div>
</section>

</div>
</article>

<div>
<!-- end index-container -->